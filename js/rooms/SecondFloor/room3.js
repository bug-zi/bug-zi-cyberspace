 ...
 */
export function createSecondFloorRoom3(scene, collisionSystem, wallMaterial, floorMaterial, ceilingMaterial, floorY, wallHeight, roomSize) {
  const roomX = 90;
  const roomZ = 0;
  ...
  // 后墙（带门洞）
  const backWallTop = new THREE.Mesh(new THREE.BoxGeometry(roomSize, wallHeight - doorHeight, wallThickness), wallMaterial);
  ...
  const backWallLeft = new THREE.Mesh(new THREE.BoxGeometry((roomSize - doorWidth) / 2, doorHeight, wallThickness), wallMaterial);
  ...
  const backWallRight = new THREE.Mesh(new THREE.BoxGeometry((roomSize - doorWidth) / 2, doorHeight, wallThickness), wallMaterial);
  ...
  // 前墙（带门洞）
  const frontWallTop = new THREE.Mesh(new THREE.BoxGeometry(roomSize, wallHeight - doorHeight, wallThickness), wallMaterial);
  ...
  const frontWallLeft = new THREE.Mesh(new THREE.BoxGeometry((roomSize - doorWidth) / 2, doorHeight, wallThickness), wallMaterial);
  ...
  const frontWallRight = new THREE.Mesh(new THREE.BoxGeometry((roomSize - doorWidth) / 2, doorHeight, wallThickness), wallMaterial);
